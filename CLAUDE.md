...
**Test runner:** always use the wrapper `C:\Tools\cmd\test85.bat` (handles PHP 8.5 / environment setup) — do **not** call `vendor/bin/tester` directly. Use absolute paths for the test directory/file.

```bash
# Run all tests
C:/Tools/cmd/test85.bat W:/Nette/Database/tests -s -C

# Run specific test directory
C:/Tools/cmd/test85.bat W:/Nette/Database/tests/Database/Explorer -s -C

# Run single test file
C:/Tools/cmd/test85.bat W:/Nette/Database/tests/Database/Explorer/Explorer.basic.phpt -s -C
```

**Database servers:** most tests connect to MySQL / PostgreSQL / MS SQL via `@dataProvider databases.ini`. Start them with Docker first (Docker Desktop must be running):

```bash
docker compose -f W:/Nette/Database/docker-compose.yml up -d
# wait until all containers report "healthy", then run the tests
```

Without the containers running, those tests fail with `Connection refused` / `could not find driver` — that's an environment issue, not a code failure.
