...
test(function() { // connect & disconnect
	...
	$preprocessor = $connection->getPreprocessor();
	...
	Assert::same($preprocessor, $connection->getPreprocessor());
	Assert::same(1, $connections);
	...
	$preprocessor2 = $connection->getPreprocessor();

	Assert::notSame($pdo, $pdo2);
	Assert::notSame($driver, $driver2);
	Assert::notSame($preprocessor, $preprocessor2);
	...
	Assert::notSame($preprocessor2, $connection->getPreprocessor());
