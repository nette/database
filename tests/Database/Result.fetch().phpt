...
test('handles cursor management in stored procedures', function () use ($connection, $driverName) {
	if ($driverName === 'mysql') {
		...
		$connection->getConnection()->getNativeConnection()->setAttribute(PHP_VERSION_ID < 80500 ? PDO::MYSQL_ATTR_USE_BUFFERED_QUERY : Pdo\Mysql::ATTR_USE_BUFFERED_QUERY, false);
